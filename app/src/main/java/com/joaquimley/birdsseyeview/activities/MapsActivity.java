...
public class MapsActivity extends FragmentActivity implements GoogleMap.OnMapLoadedCallback,
    ...
    List<LatLng> approxheatpoint = new ArrayList();
...
//                BitmapDescriptorFactory.defaultMarker(BitmapDescriptorFactory.HUE_AZURE)));
    ...
    private void addHeatMap() {
        ...
        String url = "http://35.154.25.153.xip.io/health";
        ...
        for(int i=0;i<list.size();i++){
            ...
            approxheatpoint.add(new LatLng(list.get(i).latitude,list.get(i).longitude));
    ...
    public void onLocationChanged(Location location) {
        ...
        LatLng latL = new LatLng(location.getLatitude()+0.05, location.getLongitude()+0.05);
        markerOptions1.position(latL);
        ...
        markerPoints.add(markerOptions1);
...
//            mMap.addMarker(markerOptions1);
            ...
            CameraPosition cameraPosition = new CameraPosition.Builder()
                    .target(mCurrLocationMarker.getPosition()).zoom(10).build();
    ...
    public void onMapReady(GoogleMap googleMap) {
        ...
        mMap.setOnMapClickListener(new GoogleMap.OnMapClickListener() {
            ...
            public void onMapClick(LatLng latLng) {
                ...
                if (markerPoints.size() == 1)
                { for(int j=0;j<approxheatpoint.size();j++) {

                    Log.d("condition", String.valueOf(Math.abs(approxheatpoint.get(j).latitude-latLng.latitude)<0.009));

                    if ((Math.abs(approxheatpoint.get(j).latitude-latLng.latitude)<0.009)) {
                        if(!done)
                        {
                            done = true;
                            ...
                            + getApplicationContext().getPackageName() + "/" + R.raw.notification))
                                            .setStyle(new android.support.v4.app.NotificationCompat.BigTextStyle().bigText("You are passing through the red area zone! please change your route"))
                                            .setVibrate(pattern);
...
//to post your notification to the notification bar
                            ...
                            Random r = new Random();
                            int n = r.nextInt(200-100)+100;
                            options.title("The pollution level of this area is "+n+" ppm");
                            AlertDialog alert;
                            AlertDialog.Builder alertBuilder = new AlertDialog.Builder(MapsActivity.this);
                            alertBuilder.setTitle("Alert");
                            alertBuilder.setMessage(" The pollution level of this area is "+n+" PM 10");
